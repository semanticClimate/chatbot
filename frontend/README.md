...
The frontend has been split into smaller modules while keeping the same DOM ids and API behavior:

- `js/main.js` — bootstrap only
- `js/core/` — shared DOM and API-base helpers
- `js/features/book.js` — book, encyclopedia, external modal, preview tooltip bridge
- `js/features/chat.js` — language selection, refresh, export, health, submit flow
- `js/features/sidebar.js` — sidebar toggle wiring
- `js/features/tooltip.js` — floating preview tooltip

This split is designed to be a drop-in refactor, not a rewrite.
