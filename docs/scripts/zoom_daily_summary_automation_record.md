...
| Summary tools | `backend.app/zoom_daily_summary.py` (CLI), `zoom_daily_summary_app.py` (Streamlit) |
...
| Path | Purpose |
|------|---------|
| `backend.app/zoom_daily_summary_watch.py` | Discover sessions, summarize, state file, orchestrate Slack |
| `backend.app/zoom_slack.py` | Slack file upload API (batch, size checks, session file collection) |
...
2. Python venv: `backend.app/venv`
