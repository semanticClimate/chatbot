...
| Area | Change |
|------|--------|
| **LLM system prompt** | Stronger rules: detect user language each turn; translate faithfully from English sources without extra facts; explicit support for EN, HI, FR, PT, ES (plus other languages the user writes). Files: `climate_streamlit/prompts/system_rag_json.txt`, mirrored in legacy `climate_streamlit/app.py`. |
...
| **API UX** | `GET /` on the API host returns a short HTML page explaining that this URL is the API, not the chat UI (helps Cloudflare tunnel confusion). `climate_streamlit/api_server.py`. |
...
- `climate_streamlit/prompts/system_rag_json.txt`
- `climate_streamlit/app.py` (duplicate system prompt for monolithic Streamlit path)
- `climate_streamlit/api_server.py` (`GET /` landing)
