...
Headings with `data-section-number` now show a visible prefix in the book iframe, e.g. **§ 1.2 — The greenhouse effect**, via CSS in `backend.app/assets/book_iframe_highlight.css`. Annotation stamps numbers on section title headings (nested and flat HTML).
...
**Changes:**
- `backend.app/html_sectioning.py` — one DOM block = one chunk/anchor (no short-paragraph merge in Format B collection).
- `backend.app/rag/indexing.py` — `INDEX_SCHEMA_VERSION = para-anchor-v2` rebuilds Chroma when schema changes.
...
`backend.app/llm/ask.py` — see `_ENCYCLOPEDIA_HINT` and `_retrieval_is_weak()`.
...
```
Fixed in [PR/commit] in `backend.app/assets/book_iframe_highlight.css`:
