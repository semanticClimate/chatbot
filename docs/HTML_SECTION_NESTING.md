...
1. **`annotate_html_with_section_ids()`** (`backend.app/html_sectioning.py`) assigns `data-section-number` on outline nodes and stamps the same value on each section’s title heading.
...
2. **`inject_book_viewer_assets()`** (`backend.app/rag/book_document.py`) injects `backend.app/assets/book_iframe_highlight.css` and `book_iframe_jump.js` into the HTML served to Streamlit, FastAPI (`/book/document`), and the static web client iframe.
