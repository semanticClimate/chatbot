...
# Ensure API key exists in current shell environment or load from .env.
if (-not $env:GROQ_API_KEY) {
    $envFile = Join-Path $RepoRoot "venv\.env"
    if (Test-Path $envFile) {
        $env:GROQ_API_KEY = (Get-Content -Path $envFile -ErrorAction SilentlyContinue | Where-Object { $_ -match '^GROQ_API_KEY=' } | ForEach-Object { $_ -replace '^GROQ_API_KEY=','' })
    }
    
    if (-not $env:GROQ_API_KEY) {
        throw "GROQ_API_KEY is not set in this PowerShell session or venv\.env."
    }
}
