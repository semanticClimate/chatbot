...
This document defines the logical and physical server shape for the Climate Academy chatbot shipped in `backend.app/app.py`.
...
Relative to repository root (`ROOT_DIR = parent of backend.app/`):

| Path | Purpose |
|------|---------|
| `backend.app/` | Application code (`app.py`, `html_sectioning.py`) |
