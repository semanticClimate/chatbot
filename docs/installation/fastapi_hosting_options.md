...
The Streamlit app in `backend.app/` is a separate deployment path; this document is for the **API + client** stack only.
...
When the FastAPI app exists in this repo, reuse the same RAG building blocks as `backend.app/app.py` (`html_sectioning`, Chroma paths, Groq client) behind startup-loaded singletons and thin route handlers.

---

## Related code and docs

- `backend.app/app.py` — current RAG logic (reference for a future FastAPI port)
- `backend.app/html_sectioning.py` — HTML chunking and section numbering
