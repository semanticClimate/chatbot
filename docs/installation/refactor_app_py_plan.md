# Refactor plan: `backend.app/app.py`
...
**Target:** `backend.app/assets/app_streamlit.css` (or `static/css/app.css`).
...
- `backend.app/assets/book_iframe_jump.js` (optional: minify later; start readable)
...
**Target:** `backend.app/config/app.defaults.toml` (or `yaml`) as the **single defaults** file checked into the repo. Structure sketch:
...
**Prompts:** `backend.app/prompts/system_rag_json.txt` with a `{context}` placeholder for `.format(context=...)`. Keeps the large system instruction out of Python.
...
**Naming:** adjust folder depth (`backend.app/ui/…` vs flat `ui_*.py`) to taste; rule of thumb is **one file ≈ one screen** of substantive logic.
...
The refactor described here is **implemented** in `backend.app/`:

| Area | Location |
|------|----------|
| Default config (paths, limits, UI copy) | `backend.app/config/app.defaults.toml` |
| Settings loader | `backend.app/config_loader.py` |
| Streamlit shell CSS | `backend.app/assets/app_streamlit.css` + `styling.py` |
| Iframe book CSS / JS | `backend.app/assets/book_iframe_highlight.css`, `book_iframe_jump.js` |
| System prompt template | `backend.app/prompts/system_rag_json.txt` |
| RAG / indexing | `backend.app/rag/` |
| LLM | `backend.app/llm/` |
| PDF helpers | `backend.app/pdf/` |
| UI | `backend.app/ui/` |
| Entry | `backend.app/app.py` |

### How to run and test

1. **Environment** (from repo root or `backend.app/`): activate your venv, ensure `GROQ_API_KEY` is set (`.streamlit/secrets.toml` or environment), and `pip install -r requirements.txt` plus `pymupdf` if needed.
...
4. **Change tuning without code edits**  
   Edit `backend.app/config/app.defaults.toml` (e.g. `retrieval.top_k`, `llm.model`, UI strings), save, and use **Streamlit’s “Always rerun”** or refresh the browser. If you change only TOML, a browser refresh may be enough; `get_settings()` is cached in-process—restart Streamlit to guarantee picked-up changes.
