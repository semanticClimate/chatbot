...
- Deliver a **ZIP** (or installer) containing `backend.app/`, `input/`, and documentation placeholders; **`chroma_db/` omitted** (built on first run).
...
- Scripts run `python -m venv .venv` in a **single canonical location** (repo root or `backend.app/` — choose one and document it consistently).
- `python -m pip install --upgrade pip`, then `pip install -r requirements.txt` and **`pymupdf`** per `backend.app/README.md`.

### Phase D — Secrets

- Guided creation of `backend.app/.streamlit/secrets.toml` **or** a small placeholder flow that reminds the user to set `GROQ_API_KEY`.
- Any automation around secrets should not hide risks: file permissions and plain-text storage on disk.

### Phase E — First launch

- Run `streamlit run app.py` from `backend.app/`.
...
5. If something fails, run **check_environment** or see “Common issues” in `backend.app/README.md` (to be extended).
...
- **Venv location:** repository root versus inside `backend.app/`.
- **Windows shell:** PowerShell-only versus CMD + batch fallback for broader compatibility.

When those are decided, implement the artifacts in the first table and align `backend.app/README.md` with the chosen canonical path.
