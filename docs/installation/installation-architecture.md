...
2. **Python**: Match project expectation (see `backend.app/README.md`); isolate with a venv.
...
Code resolves `ROOT_DIR` as the parent of `backend.app/`, so the app expects `input/` and default `chroma_db/` beside `backend.app/`, not inside it.
...
- From `backend.app/`: `streamlit run app.py`.
...
1. Pull or deploy new application code under `backend.app/`.
...
```bash
cd /path/to/chatbot/backend.app
...
For day-to-day developer setup, follow `backend.app/README.md` step by step.
...
1. **Prep once**  
   ...
   - Ensure `input/` book assets and `chroma_db/` behavior match `backend.app/Getting_started.md`.
