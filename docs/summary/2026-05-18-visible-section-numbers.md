...
`annotate_html_with_section_ids()` in `climate_streamlit/html_sectioning.py` already assigned `data-section-number` to outline nodes. Gaps:
...
New **`inject_book_viewer_assets()`** in `climate_streamlit/rag/book_document.py` loads the shared CSS + `book_iframe_jump.js` into annotated HTML.

Used by:

- `build_annotated_book_document()` (FastAPI `/book/document`, web client iframe)
- `climate_streamlit/rag/indexing.py` → `get_annotated_book_html()`
- `climate_streamlit/app.py` → `get_annotated_book_html()` (removed ~130 lines of duplicated inline CSS/JS)
...
| File | Change |
|------|--------|
| `climate_streamlit/html_sectioning.py` | `_section_title_heading_A()`, stamp headings in Format A, docstring |
| `climate_streamlit/assets/book_iframe_highlight.css` | Visible § `::before` rule |
| `climate_streamlit/rag/book_document.py` | `inject_book_viewer_assets()`, `package_assets_dir()` |
| `climate_streamlit/rag/indexing.py` | Use shared injector |
| `climate_streamlit/app.py` | Use shared injector (drop inline duplicate assets) |
